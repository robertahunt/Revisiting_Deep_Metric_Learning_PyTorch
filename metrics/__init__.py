...
class MetricComputer:
    ...
    def compute_standard(self, opt, model, dataloader, evaltypes, device, **kwargs):
        ...
        with torch.no_grad():
            target_labels = []
            final_iter = tqdm(
                dataloader, desc="Embedding Data...".format(len(evaltypes))
            )
            ...
            for idx, inp in enumerate(final_iter):
                ...
                if opt.debug:
                    ...
                    target_labels = np.repeat(
                        target_labels, 1 + n_images // len(target_labels), axis=0
                    )[:n_images]

                    if isinstance(out, dict):
                        feature_colls[evaltype] = np.repeat(
                            out[evaltype].cpu().detach().numpy(),
                            1 + n_images // out[evaltype].shape[0],
                            axis=0,
                        ).tolist()[:n_images]
        ...
        for evaltype in evaltypes:
            ...
            if "kmeans_cosine" in self.requires:
                ...
                centroids_cosine = faiss.vector_float_to_array(
                    kmeans.centroids
                ).reshape(n_classes, features_cosine.shape[-1])
            ...
            if "nearest_features" in self.requires:
                ...
                max_kval = np.max(
                    [int(x.split("@")[-1]) for x in self.metric_names if "recall" in x]
                )
            ...
            if "nearest_features_cosine" in self.requires:
                ...
                _, k_closest_points_cosine = faiss_search_index.search(
                    normalize(features_cosine, axis=1), int(max_kval + 1)
                )
                k_closest_classes_cosine = target_labels.reshape(-1)[
                    k_closest_points_cosine[:, 1:]
                ]
            ...
            for metric in self.list_of_metrics:
                ...
                if "kmeans_nearest_cosine" in metric.requires:
                    input_dict[
                        "computed_cluster_labels_cosine"
                    ] = computed_cluster_labels_cosine
