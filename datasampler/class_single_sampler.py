...
class Sampler(torch.utils.data.sampler.Sampler):
    ...
    def __init__(self, opt, image_dict, image_list, **kwargs):
        ...
        self.name             = 'class_single_sampler'
    ...
    def __iter__(self):
        for _ in range(self.sampler_length):
            ...
            class_keys = np.random.choice(self.classes, size=self.batch_size, replace=False)
            
            class_ix_list = [random.choice(self.image_dict[class_key])[-1] for class_key in class_keys]
