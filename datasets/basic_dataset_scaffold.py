        ...
        if not self.is_validation:
            if opt.augmentation == "base" or opt.augmentation == "big":
                ...
                self.normal_transform.extend(
                    [
                        SquarePad(),
                        transforms.RandomAffine(degrees=(-3,3),translate=(0,0.01),scale=(0.95,1.05), fill=255),
                        transforms.RandomResizedCrop(size=crop_im_size, scale=(0.75,1.5)),
                        transforms.RandomVerticalFlip(0.5),
                    ]
                )

            elif opt.augmentation == "Testing":
                self.normal_transform.extend(
                    [
                        transforms.RandomResizedCrop(size=crop_im_size),
                        transforms.RandomVerticalFlip(0.5),
